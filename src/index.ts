...
export class CSSUnitManager {
  ...
  private static customConversionRates: ConversionRates = {}; // Store dynamically added units
  ...
  static addUnit(
    unit: string,
    conversionRates: { [key: string]: number | (() => number) }
  ): void {
  ...
  static fromString(cssString: string): CSSUnitManager {
    ...
    if (!match) throw new Error("Invalid CSS string");
  ...
  toUnit(targetUnit: string): CSSUnitManager {
    ...
    if (!conversionRate) {
      throw new Error(
        `No conversion available from ${this.unit} to ${targetUnit}`
      );
    }
  ...
  add(value: number, unit: string = this.unit): this {
    const otherValue = new CSSUnitManager(value, unit).toUnit(this.unit).value;
    this.value += otherValue;
    return this;
  }

  subtract(value: number, unit: string = this.unit): this {
    const otherValue = new CSSUnitManager(value, unit).toUnit(this.unit).value;
    this.value -= otherValue;
    return this;
  }
  ...
  min(value: number, unit: string = this.unit): this {
    const otherValue = new CSSUnitManager(value, unit).toUnit(this.unit).value;
    this.value = Math.min(this.value, otherValue);
    return this;
  }

  max(value: number, unit: string = this.unit): this {
    const otherValue = new CSSUnitManager(value, unit).toUnit(this.unit).value;
    this.value = Math.max(this.value, otherValue);
    return this;
  }

  normalize(min: number = 0, max: number = 100): this {
    if (min > max) throw new Error("Min cannot be greater than Max");
